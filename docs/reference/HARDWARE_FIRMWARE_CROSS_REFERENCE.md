...
| MAT and knock recovery | Knock attack `$D0D1-$D157`; recovery/failure `$EB3A-$EBB1`; Error 43 `$E6DF-$E6EC` | `$00A5`, knock/failure flags and calibration `$C1B9-$C1D2`, `$C229-$C22A` | CAL29 is the analog ESC/U10-AN9 path; CAL32 is U12 `KNOCK`, whose output reaches U9 pin 42 | `src/mat_control.inc.h`, `src/ecm_core.inc.h`, `src/diagnostic_qualification.inc.h` | MAT, fuel/spark and diagnostic regressions | F0 software; F1 separate ESC/KNOCK connections; F2 event stimulus; F4 U12/U9 conversion |
...
| MEMCAL cylinder selection | `$F682-$F68B` compares `$002F & $18` with `LC225`; `$E6A1-$E6AA` latches Error 41; `$CDE6-$CDF5` separately uses `LC009` for RPM normalization | CAL56 network; PROM `LC009=$00`, `LC225=$00`; MPU-derived `$002F` | `CAL56 -> U12 pin 11 CYL`; U12/MPU encoding undocumented | Existing listing-backed cylinder/RPM behavior only; do not synthesize U12 | Scheduler, startup-fuel, injector and diagnostic regressions | F0 comparisons; F1 connection; strong end-to-end inference; F4 encoding |
| MEMCAL oscillator configuration | No direct processor read identified | CAL42-CAL34 91 kOhm external-terminal relationship | `CAL42 -> U11 pin 18 OSC` | Implicit custom-device boundary | Step-129 topology and Step-130 path audit | F1 connection; F4 transfer unknown |
| MEMCAL MAP/VIGN configuration | Firmware MAP/load behavior exists, but no direct CAL61/CAL59 register consequence is proven | CAL61 common with J4 64; CAL59-to-CAL55/57 24 kOhm | `CAL61 -> U11 pin 28 MAP`; `CAL59 -> 100 Ohm -> VIGN` plus capacitor | Keep raw MAP stimulus separate from any invented MEMCAL transfer | Step-129 topology and Step-130 path audit | F1 connections; F4 U11 consequence |
