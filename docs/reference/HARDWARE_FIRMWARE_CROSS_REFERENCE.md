...
| Factory test | `$C8B2-$C8DF`, `$CA6F-$CA73`, `$FC72-$FEA3` | `$0031/$0032/$0047-$004A`, `$0172-$018C`, MPU and I/O windows | FMD/SPI, A/D inputs, COP, output drivers and SWI/reset | `src/factory_test_control.inc.h`, `src/factory_test_execution.inc.h` | Step-117/118 regressions | Confirmed emitted software and raw writes; electrical effects and mode semantics remain boundaries |
| Vector and exceptional entry | `$FFF0-$FFFE`; targets `$6000/$C9F4/$F27B/$C800` | Eight emitted vector words | Processor vector assignment, reset/COP/clock consequences, external `$6000` code | `src/vector_boundaries.inc.h` | `tests/vector_boundaries_regression.inc.h` | Confirmed table, IRQ route and immediate RTI; remaining consequences are boundaries |
