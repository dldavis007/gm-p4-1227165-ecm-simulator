...
| Normal FMD discrete refresh | `$CBDC-$CC03`; `LF1E0/LF1E5` raw reads | raw replies `$002E/$002F`; decoded status `$0037` | U12/FMD SPI boundary; byte-1 bits include listing-documented P/N and A/C status after complement | `src/ecm_core.inc.h`, `src/scheduler_serial.inc.h`, raw setters in `src/hal_interface.inc.h` | Step-158 normal-FMD HAL regression | F0 refresh and status semantics; F1 FMD connection; F2 raw reply bytes; individual unresolved bit wiring and FMD internals F4 |
