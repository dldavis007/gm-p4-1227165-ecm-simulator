...
| SYS-022 | Confirmed | The normal 160-baud display stream is translated, but LF880's factory table branch `$F88E-$F8B4` and diagnostic table branch `$F8CB-$F8ED` are not: the current C routine explicitly returns when either route is selected. | `bua-hac.lst`, `src/scheduler_serial.inc.h`, Step-147 theory audit, and Step-167 completeness audit. |
