...
| `bua-hac.asm` (Drive historical source) | 447,436 bytes | `8ffdae1c94b892098e6b44ed5b380d10334ad34d37a48f9d48655e3713ec633c` | Corrected assembly source verified to correspond to the authoritative zero-error listing and generated S-record/binary outputs. See `evidence/firmware/BUA_SOURCE_PROVENANCE.md`. |
| [`evidence/firmware/bua-hac.lst`](../../evidence/firmware/bua-hac.lst) | 938,156 bytes | `5dd9df745532fae26ef6a19deb4402690e64bb8a254aeb33d10c14a1ad6dce83` | Authoritative assembled listing used for emitted bytes, addresses, targets, tables, and execution order. Its Drive copy is byte-identical to the repository file. |
| `bua-hac.s19` (Drive historical output) | 38,748 bytes | `cb579761ceca0c9d80b6403a662f7fa461d0a29be6cbe0589eb495425afa17e2` | Generated S-record verified byte-for-byte against all 16,092 bytes explicitly emitted by the authoritative listing. |
| `bua-hac.bin` (Drive historical output) | 16,384 bytes | `302cd1d2cc3ecdf7cf097085c426b9f609dfeb7a6ac8131f268cde10fbb12fc2` | Generated 16 KiB image verified against every byte explicitly emitted by the authoritative listing. |
| `BUA_from_Car.bin` (Drive historical image) | 16,384 bytes | `c3948761aab0a44578fcc539262bcca58b690142c42d5c2c7afeb64be6a3b4fd` | Historical from-car image. Differs from corrected `bua-hac.bin` at only `$C005`, `$C007`, and `$C6AE`. |
| `BUA 86 vette 5.7TPI auto with 3.07.bin` (Drive historical image) | 16,384 bytes | `9924abd4071ef1660753bf2db1aa61e6c1d815872dfbf2362725aa56341dfb31` | Historical 1986 Vette image. Differs from corrected `bua-hac.bin` only at `$C007` and `$C6AE`; the corrected source explicitly documents the `$C6AE` correction and resulting checksum change. |
| [`evidence/firmware/BUA_SOURCE_PROVENANCE.md`](../../evidence/firmware/BUA_SOURCE_PROVENANCE.md) | text record | repository-controlled | Reproducible comparison record tying corrected source, authoritative listing, S-record, generated binary, and historical PROM images together. |
...
| SYS-020 | Superseded/narrowed | Step 122 initially concluded that no known reachable and internally defined firmware region remained unported. Step 167 identified the still-bounded diagnostic and factory branches of the 160-baud LF880 byte manager, so that conclusion now applies only outside `$F88E-$F8B4` and `$F8CB-$F8ED`. | `bua-hac.lst`, Step-122 closure audit plus its Step-167 addendum, and Step-167 ALDL completeness audit. |
| SYS-021 | Confirmed | The corrected `bua-hac.asm` source corresponds to the authoritative `bua-hac.lst`; the associated S-record and binary agree with every byte explicitly emitted by the listing. | Direct source/listing structure comparison and byte-for-byte comparison recorded in `evidence/firmware/BUA_SOURCE_PROVENANCE.md`. |
| SYS-022 | Partially resolved by Step 168 | The normal 160-baud display stream is translated, but LF880's factory table branch `$F88E-$F8B4` and diagnostic table branch `$F8CB-$F8ED` are not: the current C routine explicitly returns when either route is selected. | `bua-hac.lst`, `src/scheduler_serial.inc.h`, Step-147 theory audit, and Step-167 completeness audit. |
...
| MEM-003 | Confirmed/strongly supported | The relevant MEMCAL architecture includes a 28-pin EPROM, a 14-terminal 16055375 resistor network, and a 16-terminal 16055376 resistor network on a 66-contact carrier with 33 positions per side. The physical carrier numbering and J4 interleave are established; component-to-carrier routing uses the documented mechanical correspondence. | Direct owner inspection, reverse-engineering photographs, LTspice models, KiCad reconstructions, resistor records, and historical seven/eight-pin-per-side descriptions. Targeted electrical checks remain useful, but a blanket continuity test is not required to establish numbering. |
...
| MEM-007 | Strongly supported | The 16055375 reconstruction is a 14-terminal external-terminal electrical equivalent implemented on a generic 16-position board; its zero-ohm parts are routing/configuration jumpers, with J8/J9 unused in the local package-position correspondence. | Photograph-authoritative as-built population, terminal-equivalence audit, LTspice comparison, KiCad PCB, resistance-analysis artifacts, and user construction description. A future complete terminal-resistance matrix can refine the equivalence without reopening the established carrier mapping. |
| MEM-008 | Confirmed | The 16055376 as-built reconstruction reduces to the documented 16-terminal functional network and agrees with the LTspice topology and photo-authoritative resistor values. | Completed reconstruction photograph, photo audit, topology audit, LTspice model, and reconstruction record. The conflicting KiCad 25.5-Ohm entry is retained as an erroneous design record; the installed and LTspice value is 24 kOhm. |
| MEM-009 | Strongly supported | The surviving SPICE/KiCad artifacts resolve the earlier ambiguity over which historical resistor set belongs to which network: the simple nine-value set corresponds to 16055376, while the more complex reconstruction corresponds to 16055375. | Independent project artifacts with matching pin counts, values, and topology. The 16055375 reconstruction is an external-terminal electrical equivalent and is not claimed to reproduce LTspice's internal resistor topology literally. |
...
| --- | --- | --- |
| OPEN-001 | Resolved | Physical IC-style carrier numbering and its odd/even J4 interleave are established by direct owner inspection and recorded in Step 129. Targeted continuity remains appropriate only for specific unresolved traces. |
| OPEN-002 | Partly resolved | The completed reconstructed-MEMCAL photographs are authoritative where fitted values and jumper population are legible. 16055376's functional topology and values are closed. 16055375 is an external-terminal electrical equivalent; a future complete terminal-resistance matrix can test it without blocking current theory work. |
...
| OPEN-008 | Resolved | The corrected `bua-hac.asm`, zero-error `bua-hac.lst`, generated `bua-hac.s19`/`bua-hac.bin`, and historical PROM images were located in the Drive ASM11 folder. The S-record and binary match every byte emitted by the authoritative listing; the corrected build differs from the historical 1986 Vette image only at the checksum byte and documented `$C6AE` correction. See `evidence/firmware/BUA_SOURCE_PROVENANCE.md`. |
